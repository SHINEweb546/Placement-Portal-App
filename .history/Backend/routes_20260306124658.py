...
def user_dashboard():
    ...
    return render_template("user/user_dashboard.html",jobs_list=jobs_list)
...
def view_jobs():
    ...
    return render_template("user/user_job.html", jobs=jobs)
...
def my_applications():
    ...
    return render_template(
        "user/user_application.html",
        applications=applications
    )
@app.route("/apply/<int:job_id>")
def apply_job(job_id):
    application = Application(student_id=1, job_id=job_id)
    db.session.add(application)
    db.session.commit()
    return redirect(url_for("user_dashboard"))
